...
define(['Creature','Assets'],function(Creature,Assets){
    ...
    var Explosion = Creature.extend({
        ...
        tick:function(_dt){
            ...
            this.x=this.handler.getWorld().getEntityManager().getPlayer().getX()-20;
            this.y=this.handler.getWorld().getEntityManager().getPlayer().getY()-10;


        },
        render:function(_g){

            _g.myDrawImage(this.getCurrentAnimationFrame(), this.x - this.handler.getGameCamera().getxOffset(), this.y - this.handler.getGameCamera().getyOffset(), 96, 96);
